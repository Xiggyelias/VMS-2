...
// Heuristic derivation of registrant type from email local part
$localPart = strtolower(strstr($email, '@', true) ?: '');
$derivedType = 'staff';
$confident = false;
if (preg_match('/guest|visitor/', $localPart)) {
    $derivedType = 'guest';
    $confident = true;
} elseif (preg_match('/\d/', $localPart)) {
    // Student emails often include digits
    $derivedType = 'student';
    $confident = true;
}
...
if (!$user) {
    ...
    $registrantType = $derivedType ?: 'student';
        
        // Build INSERT dynamically
        $columns = ['fullName', 'Email', 'registrantType'];
        $values = [$fullName, $email, $registrantType];
        $placeholders = ['?', '?', '?'];
    ...
    $userId = $conn->insert_id;
        ...
        // If we are not confident of the role, ask frontend to get a one-time selection
        if (!$confident) {
            echo json_encode([
                'success' => true,
...
// Determine registrantType for session
$currentType = $user['registrantType'] ?? ($registrantType ?? $derivedType ?? 'student');
