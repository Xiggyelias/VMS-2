    ...
    <script>
        ...
        function handleDriverSubmit(event) {
            ...
            .then(data => {
                hideLoading(submitButton);
                if (data.status === 'success') {
                    showAlert('success', data.message, 'driverModal');
                    setTimeout(() => {
                        location.reload();
                    }, 1500);
                } else {
                    showAlert('error', data.message || 'An error occurred. Please try again.', 'driverModal');
                }
            })
            .catch(error => {
                if (submitButton) hideLoading(submitButton);
                showAlert('error', 'An error occurred. Please try again.', 'driverModal');
                console.error('Error:', error);
            });
        ...
        function deleteDriver(driverId) {
            if (confirm('Are you sure you want to delete this driver?')) {
                ...
                .then(data => {
                    if (data.status === 'success') {
                        showAlert('success', data.message, 'driverSection');
                        setTimeout(() => {
                            location.reload();
                        }, 1500);
                    } else {
                ...
                .catch(error => {
                    console.error('Error:', error);
                    showAlert('error', error.message || 'An error occurred while deleting the driver. Please try again.', 'driverSection');
                })
                .finally(() => {
                    if (deleteButton) {
